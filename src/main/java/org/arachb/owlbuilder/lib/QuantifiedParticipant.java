...
import org.arachb.owlbuilder.lib.AbstractEntity;
...
public class QuantifiedParticipant implements AbstractNamedEntity{
	...
	private static Logger log = Logger.getLogger(QuantifiedParticipant.class);
	private final ParticipantBean bean;

	public QuantifiedParticipant(ParticipantBean b){
		bean = b;
	}
	 ...
	 */
	public static Set<QuantifiedParticipant> wrapSet(Set<ParticipantBean> bset){
		final Set<QuantifiedParticipant>result = new HashSet<QuantifiedParticipant>();
		for(ParticipantBean b : bset){
			result.add(new QuantifiedParticipant(b));
		}
		return result;
	}
